...
#include <GLFW/glfw3.h> // glfwGetProcAddress() for the airgapped GL loader
...
bool ImGui_ImplOpenGL3_InitLoader()
{
    ...
    // Airgapped build: resolve GL entry points through GLFW instead of
    // dlopen()ing the GL library (see IMGL3W_DYNLOAD in the loader header).
#if defined(GLFW_VERSION_MAJOR)
    if (glGetIntegerv == nullptr && imgl3wInit2([](const char* proc) -> GL3WglProc {
        return reinterpret_cast<GL3WglProc>(glfwGetProcAddress(proc));
    }) != 0)
#else
